    ...
    if not df.empty:
        ...
        if not approved_only_df.empty:
            ...
            with col1:
                ...
                st.download_button("Download Formatted CSV", csv_buffer.getvalue(), "formatted_payments.csv", "text/csv")
            with col2:
...
def admin_audit():
    ...
    conn = get_db_connection()
    # MODIFIED: Filters out Admin roles to only show Operator activity
    df = pd.read_sql("""
        ...
        for f in uploaded_files:
            ...
            with st.spinner(f"Processing {f.name}..."):
                ...
                try:
                    ...
                    # MODIFIED: Insert action into audit_logs table
                    cur.execute("""
                        INSERT INTO audit_logs (user_id, action) 
                        VALUES (%s, %s)""",
                                (st.session_state['user_id'], f"Uploaded Invoice: {f.name} | Status: {status}"))
...
if not st.session_state['logged_in']:
    ...
    with st.sidebar:
        ...
        # MODIFIED: Restored the Navigate Radio buttons for the Admin role
